...
- [x] Memunculkan Snackbar dengan tulisan:
  - [x] "Kamu telah menekan tombol All Products" ketika tombol All Products ditekan. -[x] "Kamu telah menekan tombol My Products" ketika tombol My Products ditekan. -[x] "Kamu telah menekan tombol Create Product" ketika tombol Create Product ditekan.
...
- [x] Membuat halaman yang berisi daftar semua item yang terdapat pada endpoint JSON di Django yang telah kamu deploy.
  - [x] Tampilkan name, price, description, thumbnail, category, dan is_featured dari masing-masing item pada halaman ini (Dapat disesuaikan dengan field yang kalian buat sebelumnya).
- [x] Membuat halaman detail untuk setiap item yang terdapat pada halaman daftar Item.
  - [x] Halaman ini dapat diakses dengan menekan salah satu card item pada halaman daftar Item.
...
- [x] Menjawab beberapa pertanyaan berikut pada README.md pada root folder (silakan modifikasi README.md yang telah kamu buat sebelumnya; tambahkan subjudul untuk setiap tugas).
  ...
  - [x] Jelaskan mengapa instance CookieRequest perlu untuk dibagikan ke semua komponen di aplikasi Flutter.
  ...
  - [x] Jelaskan mengapa kita perlu membuat model Dart saat mengambil/mengirim data JSON? Apa konsekuensinya jika langsung memetakan Map<String, dynamic> tanpa model (terkait validasi tipe, null-safety, maintainability)?
  - Model Dart diperlukan untuk memastikan type safety, validasi data, dan kemudahan maintenance kode. Tanpa model, kita akan kehilangan compile-time checking, rentan terhadap runtime errors karena akses properti yang salah, sulit melakukan refactoring, dan tidak ada intellisense yang membantu development. Map<String, dynamic> juga tidak memberikan jaminan null-safety yang bisa menyebabkan aplikasi crash.
  - [x] Apa fungsi package http dan CookieRequest dalam tugas ini? Jelaskan perbedaan peran http vs CookieRequest.
  - Package http adalah library standar untuk melakukan HTTP requests seperti GET, POST, PUT, DELETE. Sedangkan CookieRequest adalah custom class yang mengelola session dan cookie secara otomatis, terutama untuk autentikasi. Perbedaannya: http memerlukan pengelolaan cookie manual, sementara CookieRequest otomatis menyimpan dan mengirim cookie untuk maintain session login.
  - [x] Jelaskan mengapa instance CookieRequest perlu untuk dibagikan ke semua komponen di aplikasi Flutter.
  - Instance CookieRequest dibagikan menggunakan Provider agar semua widget dapat mengakses session yang sama. Ini penting untuk maintain state login pengguna di seluruh aplikasi dan memastikan konsistensi data autentikasi. Tanpa sharing instance, setiap widget akan memiliki session terpisah yang menyebabkan masalah autentikasi.
  - [x] Jelaskan konfigurasi konektivitas yang diperlukan agar Flutter dapat berkomunikasi dengan Django. Mengapa kita perlu menambahkan 10.0.2.2 pada ALLOWED_HOSTS, mengaktifkan CORS dan pengaturan SameSite/cookie, dan menambahkan izin akses internet di Android? Apa yang akan terjadi jika konfigurasi tersebut tidak dilakukan dengan benar?
  - Konfigurasi yang diperlukan: menambahkan 10.0.2.2 di ALLOWED_HOSTS untuk emulator Android, mengaktifkan CORS agar Flutter bisa akses Django dari domain berbeda, mengatur SameSite cookie untuk keamanan, dan menambahkan internet permission di Android manifest. Tanpa konfigurasi ini, Flutter tidak bisa berkomunikasi dengan Django karena CORS policy, connection refused, atau permission denied.
  - [x] Jelaskan mekanisme pengiriman data mulai dari input hingga dapat ditampilkan pada Flutter.
  - Mekanisme: User input data di Flutter → Data divalidasi dan dikonversi ke JSON → Dikirim ke Django via HTTP request → Django memproses dan menyimpan ke database → Django return response JSON → Flutter menerima response → Data di-parse ke model Dart → UI di-update untuk menampilkan data baru melalui setState atau state management.
  - [x] Jelaskan mekanisme autentikasi dari login, register, hingga logout. Mulai dari input data akun pada Flutter ke Django hingga selesainya proses autentikasi oleh Django dan tampilnya menu pada Flutter.
  - Register: Input username/password di Flutter → POST ke Django /auth/register/ → Django validasi dan buat user → Return success/error response. Login: Input credentials → POST ke /auth/login/ → Django authenticate → Set session cookie → Return user data → Flutter simpan session → Redirect ke main menu. Logout: Request ke /auth/logout/ → Django clear session → Flutter clear local session → Redirect ke login page.
  - [x] Jelaskan bagaimana cara kamu mengimplementasikan checklist di atas secara step-by-step! (bukan hanya sekadar mengikuti tutorial).
  - 1. Setup autentikasi Django dengan django-cors-headers dan authentication app. 2. Buat model Product di Flutter sesuai Django model. 3. Implementasi CookieRequest dengan Provider untuk state management. 4. Buat halaman login/register dengan validasi form. 5. Buat halaman list produk dengan FutureBuilder untuk fetch data. 6. Implementasi halaman detail produk dengan navigation. 7. Filter produk berdasarkan user yang login dengan parameter di API. 8. Test semua fitur dan fix bugs yang muncul.
